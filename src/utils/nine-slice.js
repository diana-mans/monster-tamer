 ...
 * @type {object}
 * @property {number} cornerCutSize
 * @property {Phaser.Textures.TextureManager} textureManager
...
export class NineSlice {
	...
	#createNineSliceTextures(textureManager, assetKey) {
		...
		if (texture.key === '__MISSING') {
			console.warn(
				`[${NineSlice.name}:createNineSliceTextures] the provided texture asset key was not found`,
			);
			return;
		}
		...
		if (!texture.frames['__BASE']) {
			console.warn(
				`[${NineSlice.name}:createNineSliceTextures] the provided texture asset key does not have a base texture`,
			);
			return;
		}
		...
		if (texture.getFrameNames(false).length !== 0) {
			console.debug(
				`[${NineSlice.name}:createNineSliceTextures] the provided texture asset key already has additional frames`,
			);
			return;
		}
	...
	createNineSliceContainer(scene, targetWidth, targetHeight, assetKey) {
		...
		tl.setData('assetCutFrame', ASSET_CUT_FRAMES.TL);
		...
		tm.setData('assetCutFrame', ASSET_CUT_FRAMES.TM);

		const tr = scene.add
			.image(tl.displayWidth + tm.displayWidth, 0, assetKey, ASSET_CUT_FRAMES.TR)
			.setOrigin(0);
		tr.setData('assetCutFrame', ASSET_CUT_FRAMES.TR);
		...
		ml.setData('assetCutFrame', ASSET_CUT_FRAMES.ML);
	...
	updateNineSliceContainerTexture(textureManager, container, assetKey) {
		...
		if (texture.key === '__MISSING') {
			console.warn(
				`[${NineSlice.name}:updateNineSliceContainerTexture] the provided texture asset key was not found`,
			);
			return;
		}
		if (texture.getFrameNames(false).length === 0) {
			console.debug(
				`[${NineSlice.name}:updateNineSliceContainerTexture] the provided texture asset does not have the required nine slice frames`,
			);
			return;
		}
		container.each((gameObject) => {
			...
			const cutFrame = phaserImageGameObject.getData('assetCutFrame');
			if (cutFrame === undefined) return;
			phaserImageGameObject.setTexture(assetKey, cutFrame);
		});
	}
}
